...
float Kp=0, Ki=0, Kd=0; // default values
QuickPID throttle_PID(&throttle_input, &servo_output, &throttle_setpoint, Kp, Ki, Kd, throttle_PID.Action::reverse);

sTune tuner = sTune(&throttle_input, &servo_output, tuner.zieglerNicholsPID, tuner.directIP, tuner.printALL);
...
const float outputStart = 10;
const float outputStep = 1;
